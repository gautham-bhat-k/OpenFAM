...
/*---------------------------------------------------------------------------*/
int
mercury_server_init(int argc, char *argv[], struct hg_test_info &init_info)
{
    ...
    if (hg_test_info.na_test_info.max_contexts > 1) {
    	...
    	cout << "Max contexts------- : " << context_count <<  endl;
        hg_uint8_t i;

        progress_workers =
            (struct hg_test_worker *)malloc(sizeof(struct hg_test_worker) * context_count);
        ...
        int rt = pthread_create(&hg_progress_tid, NULL, hg_test_progress_work, &progress_workers[0]);
	cout << "Progress thread created..." << endl;
	goto done;
    } else {
        hg_thread_t progress_thread;

	progress_workers = (struct hg_test_worker *)malloc(sizeof(struct hg_test_worker));
	...
	progress_workers->hg_class = hg_test_info.hg_class;
        ...
        cout << "Creating a progress thread ....." << endl;
        int rt = pthread_create(&hg_progress_tid, NULL, hg_test_progress_work, progress_workers);
	cout << "Only one context....." << endl;
	goto done;
#if 0
	hg_thread_create(
	...
	hg_thread_t progress_thread;
	hg_thread_create(
            &progress_thread, hg_test_progress_thread_single, hg_test_info.context);
	cout << "Single thread for progress...." << endl;
	goto done;
#if 0
    do {
        unsigned int actual_count = 0;
